...
#FUNCTION used for generating REGIONS. Lines 54-57.
def which_region(position, total_length, number_of_regions):
    region_length = total_length / number_of_regions
    region = math.ceil(position / region_length)
    return region


#CLASS containing all the function definitions required for calculating Kiln Surface Radiation. Lines 61-97.
class Kiln():
...
with st.sidebar:
    ...
    options = option_menu(
        ...
        options = ["Introduction", "User Notes", "Average Values", "Split DataFrames", "Fan Flows", "Cooler Fans", "Treya", "Cooler Heat Balance", "Kiln Radiation", "Kiln Heat Balance", "Dashboard Reset"],
...
#Code for the "INTRODUCTION" section. Lines 125-148.     
...
#Code for the "USER NOTES" section. Lines 152-218.
...
7. Next, what are the input columns? Scroll down below where you can see the **'GENERAL INFORMATION ABOUT YOUR DATA'** section. Before you download any of the excel sheets or view any of the dataframes, make sure to fill in the right information. Below that you may be able to see the **'FAN FLOWS'** and **'COOLER FANS'** columns. You can enter data in these expanders by clicking on them.
...
""")
    ...
    st.write("You are all set to use this app now. Go ahead and upload your excel files!!!")      
...
#EXPANDER to input the GENERAL INFORMATION required for further computation. Lines 222-246.  
...
with st.expander("GENERAL INFO"):
     ...
     backcalcordirect = st.number_input('Are you back calculating Cooler Vent Air from ESP stack or measuring it directly? (Type 1 for direct, 0 for back calculation). (Ignore this tab you have entered 0 above).', 0, 1, 0, 1)
...
#Code for the template excel that will be generated via link. Lines 249-282.       
...
if (number_of_cooler_ventair_ducts > 0 and backcalcordirect == 1):
...
elif ( (number_of_cooler_ventair_ducts > 0 and backcalcordirect == 0) or (number_of_cooler_ventair_ducts == 0) ):
...
#Lines 286-289 take care of the first output of the program, that is the appended excel with average values (columnwise).
...
#Splitting the original dataframe for further usage. Did the splitting using .T feature for transpose and the iloc function for accessing specific rows. Lines 293-366.
...
#Code for the COLUMNS OF THE DASHBOARD (excluding the CONTAINER). Lines 346-979.    
col1, col2, col3 = st.columns(3)  


#Code for COLUMN 1, taking care of the FAN FLOWS. Lines 350-703 as of now.
with col1:
    st.header("FAN FLOWS")
    ...
    with st.expander("TEMPERATURES"):                                          #EXPANDER 1 for TEMPERATURES.
         ...
         while ( (CountColumn1Expander1CoolerMidair <= number_of_coolermidair) and (number_of_coolermidair > 0) ):
                ...
                CountColumn1Expander1CoolerMidair += 1       
         if ( backcalcordirect == 1 and number_of_cooler_ventair_ducts > 0 ):
    ...
    with st.expander("DENSITY CALCULATIONS"):                               #EXPANDER 4 for DENSITY CALCULATIONS.
         st.write("In order to calculate the density of Pre Heater Downcomer Ducts, answer the following questions: (**Enter 0** in the H20 Column if you are **not** considering water vapor).")
...
#Lines 526-553 take care of the ESP sheet and it's required dataframe. And the link for downloading the excel as well.    
    if input_excel is not None:
        i = 1 
        np.ESPnamesarray = []
        while(i <= 1):
...
#Lines 556-584 take care of the TAD sheet and it's required dataframe. And the link for downloading the excel as well.
    if input_excel is not None and (number_of_tertiaryair_ducts > 0):    
        ...
        dfTAD['Area'] = (((np.pi)*dfTAD['Diameter']**2)/4)*0.9            #Have to multiply by 0.9???
...
#Lines 587-615 take care of the CMA (or WHRS o/I) sheet and it's required dataframe. And the link for downloading the excel as well.
...
#Lines 618-647 take care of the PH Downcomer Ducts sheet and it's required dataframe. And the link for downloading the excel as well.    
    if input_excel is not None:
        i = 1 
        np.PHnamesarray = []
        ...
        dfPH['Density'] = dfdensitydata['Density'] 
...
#Lines 653-703 take care of the CVA fans data (DIRECT CALCULATION, BACK CALCULATION and ZERO case as well).   
    if input_excel is not None and (number_of_cooler_ventair_ducts > 0 and backcalcordirect == 1):
        i = 1 
        np.CVAnamesarray = []
        while(i <= 1):
        ...
        dfCVA = pd.DataFrame([[np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns = ['Name', 'Sqrt DP', 'Temp', 'Temp K', 'Static Pressure', 'Calculated Pressure', 'Temp CF', 'Density', 'Diameter', 'Area', 'V', 'Flow m3/s', 'Flow m3/hr', 'Nm3/hr', 'Nm3/kgcl'])
    elif input_excel is not None and (number_of_cooler_ventair_ducts > 0 and backcalcordirect == 0):
          i = 1
          np.CVAnamesarray = []
          while(i <= 1):
              (np.CVAnamesarray).append("Cooler Vent Air %d" %i)
          ...
          with st.expander("COOLER VENT AIR BACK CALCULATION"):
               ...
               st.write("Cooler Vent Air Flow is (**X**):")
               st.write(float(Fbhai[0]))
               st.write("False Air Flow is (**Y**):") 
               ...
               #st.write("***") CHANGE this BRO!!!
        
    if input_excel is not None: 
        dfnewCVAaswell = pd.concat([dfESP, dfTAD, dfCMA, dfPH, dfCVA])
        

#Code for COLUMN 2, taking care of the COOLER FANS. Lines 707-803 as of now.  
with col2:
    ...
    if input_excel is not None and (number_of_cooler_fans > 0):  
       ...
       dfCF['Design Flow m3/hr'] = dfCF['m3/hr']/60 
       dfCF['CCR Flow'] = dfCF['m3/hr']
       dfCF['A'] = dfCF['CCR Flow']-dfCF['m3/hr']
       ...
       dfCF = pd.DataFrame()
       dfCF = pd.DataFrame([[np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns = ['Name', 'Anemometer Y/N', 'Average Velocities', 'Areas', 'Static Pressures', 'm3/sec', 'm3/hr', 'TCF', 'Nm3/hr', 'Nm3/s', 'Nm3/kgcl', 'Temp', 'Design Flow m3/hr', 'CCR Flow', 'A', 'Outlet Pressure', 'Power', '% Efficiency']) 


#Code for Column 3, taking care of the COOLER HEAT BALANCE. Lines 807-900 as of now. 
with col3:
    st.header("COOLER HB")
...
#Code for COOLER HEAT BALANCE INPUT SECTION. Lines 814-818.
    if input_excel is not None:    
...
#Code for COOLER HEAT BALANCE OUTPUT SECTION. Lines 822-914 as of now.
    if input_excel is not None:
...
#TAD HEAT BALANCE OUTPUT. Lines 830-846. 
    if input_excel is not None and (number_of_tertiaryair_ducts > 0):
        i = 1
...
#CMA HEAT BALANCE OUTPUT. Lines 849-865. 
    if input_excel is not None and (number_of_coolermidair > 0):
        i = 1
...
#CVA HEAT BALANCE OUTPUT. Lines 868-885.        
    if input_excel is not None and (number_of_cooler_ventair_ducts > 0):
        ...
        HeatBalanceOutputDataCVA['Flow (kg/kg clinker)'] = dfCVA['Nm3/kgcl']*cooler_air_density if backcalcordirect == 1 else Fbhai[0]
        HeatBalanceOutputDataCVA['Temp (Celsius)'] = dfCVA['Temp'] if backcalcordirect == 1 else CVAbackcaltemp 
        HeatBalanceOutputDataCVA['Cp (kcal/kg celsius)'] = 0.237+(23*dfCVA['Temp']*10**(-6)) if backcalcordirect == 1 else 0.237+(23*CVAbackcaltemp*10**(-6))
...
#HEAT BALANCE OUTPUT (excluding Secondary Air). Lines 887-889.        
    if input_excel is not None:     
...
#SECONDARY AIR CALCULATIONS. Line 892-902.   
...
#Code for the CONTAINER. Including TREYA code and KILN HEAT balance. Lines 904-1023 as of now.
with st.container():
...
#Code for "KILN HEAT BALANCE". Lines 1027- as of now.    
    with col5:
       st.header("KILN HB")
...
#Code for KILN RADIATION table. Lines 1090-1202.      
...
#OUTPUT HEAT KILN BALANCE Code. Lines 1112-        
       if (input_excel and input_excel_kilnsurftemps) is not None:
        KHBOutputDataSame = [
                            [ 'Heat of Reaction', np.nan, np.nan, np.nan, np.nan, np.nan, HOR],
                            [ 'Kiln Radiation', np.nan, np.nan, np.nan, np.nan, np.nan, df['Total Loss'].sum(axis = 0)],
        ...
        dfKHBOutputSame = pd.DataFrame(KHBOutputDataSame, columns = ['HEAT OUT', 'Temp', 'Delt T', 'Cp', 'Volume or Mass', 'Density', 'kcal/kg cl'])
        
#Code for PH Return Dust Dataframe and then for PH exit gas DataFrame. Lines        
       if input_excel is not None:
        i = 1
        np.PHDustnamesarray = []
        ...
        while(i <= number_of_preheater_downcomer_ducts):
             ...
             (np.PHExitGasnamesarray).append("PH Exit Gas-ILC-%d" %i)   
             i+=1
        dfPHDustnames = pd.DataFrame(np.PHDustnamesarray, columns = ['HEAT OUT']) 
        dfPHExitGasnames = pd.DataFrame(np.PHExitGasnamesarray, columns = ['HEAT OUT'])
        
        dfPHReturnQtyDust = pd.DataFrame(np.arrayforreturnqtydust, columns = ['Return Qty Dust'])
        
        KHBOutputDust = pd.DataFrame()
        KHBOutputDust['HEAT OUT'] = dfPHDustnames
        ...
        KHBOutputExitGas['HEAT OUT'] = dfPHExitGasnames
...
#Code for Mid Air DataFrame. Lines 
       if input_excel is not None and (number_of_coolermidair > 0):
        i = 1;
        np.midairnamesarray = []
        while(i <= number_of_coolermidair):
             ...
             i+=1
        dfmidairnames = pd.DataFrame(np.midairnamesarray, columns = ['HEAT OUT']) 
        
        KHBOutputCMA = pd.DataFrame()
        KHBOutputCMA['HEAT OUT'] = dfmidairnames
        ...
        KHBOutputCMA = pd.DataFrame()
        KHBOutputCMA = pd.DataFrame([['Mid Air', np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns = ['HEAT OUT', 'Temp', 'Delt T', 'Cp', 'Volume or Mass', 'Density', 'kcal/kg cl'])
       ...
       if input_excel is not None and (number_of_cooler_ventair_ducts > 0): 
        ...
        while(i <= number_of_cooler_ventair_ducts):
             ...
             i+=1
        dfcoolerventnames = pd.DataFrame(np.coolerventnamesarray, columns = ['HEAT OUT'])     
        KHBOutputCVA = pd.DataFrame()
        KHBOutputCVA['HEAT OUT'] = dfcoolerventnames
        KHBOutputCVA['Temp'] = dfCVA['Temp'] if backcalcordirect == 1 else CVAbackcaltemp
        KHBOutputCVA['Delt T'] = KHBOutputCVA['Temp']-REFERENCE_TEMPERATURE_NTP
        KHBOutputCVA['Cp'] = 0.237+(23*KHBOutputCVA['Temp']*10**-6)
        KHBOutputCVA['Volume or Mass'] = dfCVA['Nm3/kgcl'] if backcalcordirect == 1 else Fbhai[0]/cooler_air_density
        ...
        KHBOutputCVA = pd.DataFrame([['Cooler Vent', np.nan, np.nan, np.nan, np.nan, np.nan, np.nan]], columns = ['HEAT OUT', 'Temp', 'Delt T', 'Cp', 'Volume or Mass', 'Density', 'kcal/kg cl'])
...
#Code for KILN HEAT BALANCE INPUT.
       if (input_excel and input_excel_kilnsurftemps) is not None:
        ...
        dfKHBInput = pd.DataFrame(KHBInputDataSame, columns = ['HEAT IN', 'Temp', 'Delt T', 'Cp', 'Volume or Mass', 'Density', 'kcal/kg cl'])        


#Code for the "AVERAGE VALUES" section. Lines 1206-1231.
...
#Code for the "SPLIT DATAFRAMES" section. Lines 1234-1314.
...
#Code for the "FAN FLOWS" section. Lines 1318-1405.
    ...
    if input_excel is not None and (number_of_cooler_ventair_ducts == 0 or (backcalcordirect == 0 and number_of_cooler_ventair_ducts > 0)): 
...
#Code for the "COOLER FANS" section. Lines 1409-1429.
elif(options == 'Cooler Fans' and number_of_cooler_fans > 0):   
    st.title("COOLER FANS")
    
    if input_excel is not None:
       dfCF.loc[len(dfCF)] = [np.nan, np.nan, np.nan, 'Total Cooler Flow:', np.nan, np.nan, dfCF['m3/hr'].sum(axis = 0), np.nan, dfCF['Nm3/hr'].sum(axis = 0), np.nan, dfCF['Nm3/kgcl'].sum(axis = 0), np.nan, np.nan, dfCF['CCR Flow'].sum(axis = 0), np.nan, np.nan, dfCF['Power'].sum(axis = 0), np.nan]
       dfCF.loc[len(dfCF)] = [np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, dfCF['CCR Flow'].sum(axis = 0)-dfCF['m3/hr'].sum(axis = 0), np.nan, np.nan, np.nan, np.nan] 
...
#Code for "TREYA" section. Lines 1433-1496 as of now.
elif(options == "Treya"):
    st.title("TREYA")
    ...
    if input_excel is not None: 
      ...
      viewtreyasheetyesorno = st.radio(
                          "Do you want to view the Treya sheet in the form of a table?",
                            ('No','Yes'))
...
#Code for "COOLER HEAT BALANCE" section. Lines 1500-1530.
...
#Code for "KILN RADIATION" section. Lines 1537-1554.    
...
#Code for "KILN HEAT BALANCE" section. Lines 1558-1586 as of now. 
...
#https://pythoninoffice.com/replicate-excel-goal-seek-in-python/       Binary Search for GoalSeek in Python.    
#https://github.com/DrTol/GoalSeek_Python#:~:text=You%20can%20use%20the%20Goal,result%2Foutput%20(goal). NumPy Goalseek GitHub code.
#No code for dashboard reset. Just a simple hack ;).    
#https://icons.getbootstrap.com/        Link for bootstrap icons.    
#https://imgur.com/a/1PYHOo9            Link for example excel file. This link now lite. Using google drive link now. LOL!
#https://pypi.org/project/streamlit-modal/      Link for modal window support in python.  
#https://www.codegrepper.com/code-examples/python/how+to+use+variable+from+another+function+in+python  
#https://www.projectpro.io/projects           DataScience Projects.